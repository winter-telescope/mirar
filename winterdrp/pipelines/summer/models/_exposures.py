...
class RawTable(SummerBase):  # pylint: disable=too-few-public-methods
    ...
    # expid = Column(Double, primary_key=True, autoincrement=False)
    expid = Column(Double, unique=True) # Unique but deterministic
    # UNIQUE key determined from images to ensure no duplicacy
    # expcount = Column(Integer, Sequence("raw_rawcount_seq", start=1))
    uexpid = Column(Integer, primary_key=True)
    # filtername = Column(VARCHAR(20), unique=True)
    fid = Column(Integer, unique=True) #FK references filters

    # refers to night date unique key.
    ...
    progID: Mapped[int] = mapped_column(ForeignKey("programs.puid"))
