...
from winterdrp.pipelines.wirc.wirc_files import wirc_file_dir, sextractor_reference_config, psfex_path
...
def wirc_reference_image_resampler(**kwargs) -> Swarp:
    return Swarp(swarp_config_path=wirc_file_dir.joinpath("config.swarp"),
                 cache=True, **kwargs
                 )


def wirc_reference_sextractor(output_sub_dir: str, gain: float) -> Sextractor:
    return Sextractor(**sextractor_reference_config,
                      gain=gain,
...
def wirc_reference_psfex(output_sub_dir: str, norm_fits: bool) -> PSFex:
    return PSFex(config_path=psfex_path,
                 output_sub_dir=output_sub_dir,
                 norm_fits=norm_fits,
                 cache=True
                 )