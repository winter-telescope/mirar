...
class SkyMapper(BaseCatalog):
    ...
    def get_catalog(
        ...
        logger.info(
            f'Querying Skymapper catalog around RA {ra_deg:.4f}, '
            f'Dec {dec_deg:.4f} with a radius of {self.search_radius_arcmin:.4f} arcmin'
        )

        v = Vizier(columns=['*'],
                   column_filters={f"{self.filter_name}PSF": f"< {self.max_mag}",
                                   f"e_{self.filter_name}PSF": "<%.3f" % (1.086 / self.snr_threshold)},
                   row_limit=-1)
        Q = v.query_region(SkyCoord(ra=ra_deg, dec=dec_deg, unit=(u.deg, u.deg)),
                           radius=str(self.search_radius_arcmin) + 'm',
                           catalog=self.catalog_vizier_code, cache=False)

        if len(Q) == 0:
            logger.info('No matches found in the given radius in Skymapper')
            t = Table()
        else:
            t = Q[0]
            t['ra'] = t['RAICRS']
            t['dec'] = t['DEICRS']
            t['magnitude'] = t[f'{self.filter_name}PSF']
            logger.info(f'{len(t)} matches found in the given radius in Skymapper')
