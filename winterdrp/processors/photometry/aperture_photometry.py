    ...
    def __init__(
        ...
        **kwargs,
        ...
        if bkg_in_diameters is None:
            bkg_in_diameters = [25.0]
        if bkg_out_diameters is None:
            bkg_out_diameters = [40.0]
            ...
            for ind, aper_diam in enumerate(self.aper_diameters):
                ...
                for cand_ind in range(len(candidate_table)):
                    ...
                    diff_cutout = make_cutouts(
                        diff_filename, (ximage, yimage), self.cutout_size_aper_phot
                    )
                    diff_unc_cutout = make_cutouts(
                        diff_unc_filename, (ximage, yimage), self.cutout_size_aper_phot
                    )

                    flux, fluxunc = self.aper_photometry(
                        diff_cutout,
                        diff_unc_cutout,
                        aper_diam,
                        bkg_in_diameter,
                ...
                candidate_table[f"sigmagap{suffix}"] = 1.086 * (
                    candidate_table[f"fluxuncap{suffix}"]
