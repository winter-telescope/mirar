...
from mirar.pipelines.spring.models._exposures import ExposuresTable
from mirar.pipelines.spring.models.base_model import SPRINGBase

DEFAULT_FIELD = 999999999


class FieldsTable(SPRINGBase):  # pylint: disable=too-few-public-methods
    """
...
class FieldEntry(BaseDB):
    ...
    # TODO : Need to fix formatting of fields file in wintertoo before including these
...
def populate_fields():
    ...
    if not is_populated(FieldsTable):
        ...
        keys = list(FieldEntry.model_fields)
        ...
        for k, i in tqdm(enumerate(idx[:-1]), total=len(idx) - 1):
            ...
            stmt = Insert(FieldsTable).values(res[keys].to_dict(orient="records"))
