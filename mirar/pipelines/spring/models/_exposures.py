...
from mirar.pipelines.spring.models._nights import Night, NightsTable
...
class ExposuresTable(SPRINGBase):  # pylint: disable=too-few-public-methods
    """
    ...
    # raws: Mapped["RawsTable"] = relationship(back_populates="exposures")
...
class Exposure(BaseDB):
    """
    A pydantic model for an exposure database entry
    """
    ...
    fid: int = fid_field
    nightdate: date = Field()  # TODO: is this different to obsdate?
    fieldid: int = fid_field
    itid: int = Field(ge=0)
    ...
    def exists(self) -> bool:
        """
        Checks if the pydantic-ified data exists in the corresponding sql database
