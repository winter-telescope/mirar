...
class SuperSourcesTable(WinterBase):  # pylint: disable=too-few-public-methods
    ...
    sourceid = Column(
        BigInteger,
        Sequence(name="supersources_candid_seq", start=1, increment=1),
        unique=True,
        autoincrement=True,
        primary_key=True,
    )
    ...
    supercandidates: Mapped[List["SuperCandidatesTable"]] = relationship(
        back_populates="supersource"
    )


class SuperSource(BaseDB):
    """
    A pydantic model for a source database entry
    """

    sql_model: ClassVar = SuperSourcesTable
