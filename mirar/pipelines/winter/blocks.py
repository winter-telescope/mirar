...
from mirar.processors.mask import WriteMaskedCoordsToFile
from mirar.pipelines.winter.load_winter_image import load_raw_winter_image, \
    load_proc_winter_image, load_stacked_winter_image
...
from mirar.processors.utils import ImageDebatcher, ImageSaver, ImageLoader, \
    ImageSelector, ImageBatcher
...
from mirar.pipelines.winter.config import sextractor_astrometry_config, \
    sextractor_autoastrometry_config, swarp_config_path, sextractor_photometry_config
...
board_id = 5
target_name = "M101"
split = [MultiExtParser(input_sub_dir="raw/",
                        extension_num_header_key="BOARD_ID",
                        only_extract_num=board_id,
                        output_sub_dir=f"raw_split_{board_id}")]
load = [ImageLoader(input_sub_dir=f"raw_split_{board_id}",
                    load_image=load_raw_winter_image),
        ImageSelector(("OBSTYPE", ["FOCUS", "DARK", "FLAT", "SCIENCE"])), ]

load_proc = [ImageLoader(input_sub_dir=f"skysub_{board_id}",
                         load_image=load_raw_winter_image),
             ImageSelector(("TARGNAME", f"{target_name}"), ("OBSTYPE", "SCIENCE")),
             ]

load_dark = [ImageLoader(input_sub_dir=f"darkcal_{board_id}",
                         load_image=load_raw_winter_image),
             ImageSelector(("TARGNAME", f"{target_name}"))]

load_anet = [ImageLoader(input_sub_dir=f"anet_{board_id}",
                         load_image=load_proc_winter_image),
             ImageSelector(("TARGNAME", f"{target_name}"), ("OBSTYPE", "SCIENCE"))]

load_stack = [ImageLoader(input_sub_dir=f"anet_{board_id}",
                          load_image=load_proc_winter_image),
              ImageSelector(("TARGNAME", f"{target_name}"), ("OBSTYPE", "SCIENCE")),
              ImageBatcher("EXPTIME")]

load_multiboard_stack = [ImageLoader(input_sub_dir=f"stack_all_{target_name}",
...
flat_cal = [
    ...
    SkyFlatCalibrator(flat_mask_key=FITS_MASK_KEY,
                      cache_sub_dir=f'skycals_{board_id}'),
...
process_proc = [ImageDebatcher(),
                AstrometryNet(output_sub_dir=f'anet_{board_id}',
                              scale_bounds=[25, 40],
                              scale_units='amw',
                              use_sextractor=True,
                              parity='neg',
                ...
                ImageDebatcher(),
                Swarp(swarp_config_path=swarp_config_path, calculate_dims_in_swarp=True,
                      include_scamp=True,
                      subtract_bkg=False,
                      cache=True,
                      center_type="ALL",
...
stack_proc = [ImageBatcher(["TARGNAME", "FILTER"]),
              Swarp(swarp_config_path=swarp_config_path, calculate_dims_in_swarp=True,
                    include_scamp=False,
                    subtract_bkg=True,
                    cache=True,
              ...
              Sextractor(**sextractor_photometry_config,
                         output_sub_dir=f"phot_{board_id}_{target_name}",
                         checkimage_type="BACKGROUND_RMS"),
              PhotCalibrator(ref_catalog_generator=winter_photometric_catalog_generator,
                             temp_output_sub_dir=f"phot_{board_id}_{target_name}",
                             write_regions=True),
              ImageSaver(output_dir_name=f"phot_{board_id}_{target_name}"),
              ]

stack_multiboard = [Swarp(swarp_config_path=swarp_config_path,
