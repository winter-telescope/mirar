"""
Module for loading raw LRIS images and ensuring they have the correct format
"""
...
from mirar.pipelines.lris.config.constants import (
    LRIS_FILTERS,
    LRIS_GAIN,
    LRIS_NONLINEAR_LEVEL,
)

logger = logging.getLogger(__name__)


def load_raw_lris_fits(path: str | Path) -> tuple[np.array, astropy.io.fits.Header]:
    """
    Function to load a raw LRIS image
    ...
    if GAIN_KEY not in header.keys():
        header[GAIN_KEY] = LRIS_GAIN

    header["FILTER"] = header["FILTER1"].lower().strip().split("-")[-1]

    assert header["FILTER"] in LRIS_FILTERS, f"Filter {header['FILTER']} not recognised"
    ...
    if SATURATE_KEY not in header:
        header[SATURATE_KEY] = LRIS_NONLINEAR_LEVEL * header["DETCOADD"]
...
def load_raw_lris_image(path: str | Path) -> Image:
    """
    Function to load a raw LRIS image
    ...
    :return: Image object
    """
    return open_raw_image(path, load_raw_lris_fits)
