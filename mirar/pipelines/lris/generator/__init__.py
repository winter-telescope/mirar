...
from mirar.pipelines.lris.config.constants import LRIS_WIDTH
from mirar.pipelines.lris.generator.stacks import label_stack_id
from mirar.pipelines.lris.generator.target import annotate_target_coordinates
...
LRIS_SEARCH_RADIUS_ARCMIN = LRIS_WIDTH
LRIS_PHOTOMETRIC_MAX_MAG = 22


def lris_astrometric_catalog_generator(image: Image) -> Gaia2Mass:
    """
    ...
    cat = Gaia2Mass(
        min_mag=10,
        max_mag=24,
        search_radius_arcmin=LRIS_SEARCH_RADIUS_ARCMIN,
        trim=True,
...
def lris_photometric_catalog_generator(image: Image) -> BaseCatalog:
    """
    ...
    if filter_name in ["u", "U"]:
        if in_sdss(ra, dec):
            return SDSS(
                min_mag=10,
                max_mag=LRIS_PHOTOMETRIC_MAX_MAG,
                search_radius_arcmin=LRIS_SEARCH_RADIUS_ARCMIN,
...
def lris_reference_image_generator(image: Image) -> BaseReferenceGenerator:
    """
...
def lris_reference_image_resampler(**kwargs) -> Swarp:
    """
...
def lris_sdss_reference_cat_purifier(catalog, image: Image):
    """
    Purify SDSS catalog
    :param catalog:
    :param image:
...
def lris_reference_psfex(output_sub_dir: str, norm_fits: bool) -> PSFex:
    """
...
def lris_zogy_catalogs_purifier(sci_catalog, ref_catalog):
    """
