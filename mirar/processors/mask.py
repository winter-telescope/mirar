...
from mirar.paths import BASE_NAME_KEY, get_output_dir, CSV_MASK_KEY
    ...
    def _apply_to_images(
        ...
        for image in batch:
            ...
            if self.write_masked_pixels_to_file:
                ...
                masked_pixels_wcs = wcs.pixel_to_world(masked_pixels_indices[1],
                                                       masked_pixels_indices[0])
                ...
                mask_file_path = mask_directory / f"{image[BASE_NAME_KEY]}" \
                                                  f"_masked_pixels_wcs.csv"
...
class MaskPixelsFromPath(BaseMask):
    ...
    def __init__(self, mask_path: str | Path = None,
                 ...
                 write_masked_pixels_to_file: bool = False,
                 output_dir: str | Path = "mask"):
        super().__init__(write_masked_pixels_to_file=write_masked_pixels_to_file,
...
class MaskPixelsFromWCS(BaseMask):
    ...
    def __str__(self) -> str:
        return f"Processor to mask pixels using a  list of RA/Dec: "
    ...
    def get_mask(self, image) -> np.ndarray:
        ...
        mask_in_image = np.logical_and(mask_pixels_x >= 0,
                                       mask_pixels_x < mask.shape[1]) \
                        & np.logical_and(mask_pixels_y >= 0,
                                         mask_pixels_y < mask.shape[0])
        logger
...
class WriteMaskedCoordsToFile(BaseMask):
    ...
    def __init__(self, output_dir: str | Path = "mask"):
        super().__init__(write_masked_pixels_to_file=True,
                         output_dir=output_dir)
